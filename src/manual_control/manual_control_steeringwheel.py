...
class World(object):
    ...
    def spawn_vehicle(self, vehicle_index=0, spawn_index=0, x_offset=0, y_offset=0, pattern='vehicle.*model3*', rotation=None) -> carla.Vehicle:
    ...
    def toggle_radar(self):
        if self.radar_right is None and self.radar_left is None:
            self.radar_left = RadarSensor(self.player, "left", y=-1, pitch=5, yaw=-45)
            self.radar_right = RadarSensor(self.player, "right", y=1, pitch=5, yaw=45)

            self.radar_right.start_timer(3)
            self.radar_left.start_timer(3)

        else:
            self.radar_left.sensor.destroy()
            self.radar_left = None
            self.radar_right.sensor.destroy()
            self.radar_right = None
